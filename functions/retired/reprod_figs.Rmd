---
title: Cost-effectiveness analysis of public health interventions against RSV in children under 5 years in Kenya
  and South Africa
...
kenya_ari_sari_incidence <- read_csv("custom_input/kenya_ari_sari_incidence.csv")
...
ggplot(kenya_ari_sari_incidence, aes(x=age_in_months)) + 
  ...
  geom_errorbar(
    aes(ymin=popul_denom*CI_95_lower/metric_per_popul,ymax=popul_denom*CI_95_upper/metric_per_popul),size=0.4) +
  ...
  theme_bw() + standard_theme + theme(axis.text.x=element_text(vjust=0.5,size=8),
               ...
               legend.background=element_rect(fill=NA),legend.position="bottom", # c(0.92,0.925)
...
if (save_flag){
ggsave(
  paste0("output/cea_plots/kenya_ari_sari_burden_errorbars_grouped_",
         ifelse(popul_denom>1,"per100k",""),".png"),
...
deaths_data <- read_csv("custom_input/deaths_data.csv")
# bind_rows(deaths_SA %>% select(!age_inf) %>% group_by(age_in_months_orig,in_hospital) %>%
...
ggplot(deaths_data,aes(x=age_in_months_orig)) + 
  ...
  theme(axis.text.x=element_text(vjust=0.5,size=11),axis.text.y=element_text(size=11),
    legend.text=element_text(size=13),legend.background=element_rect(fill=NA),
    strip.text=element_text(size=14),legend.position=c(0.88,0.925),
...
ggplot(SA_ILI_SARI_rawdata,aes(x=age)) + 
  ...
  theme(axis.text.x=element_text(vjust=0.5,size=8),axis.text.y=element_text(size=8),
     legend.text=element_text(size=7),legend.background=element_rect(fill=NA),
     legend.position="bottom", axis.title.x=element_text(size=10),
     axis.title.y=element_text(size=10),strip.text=element_text(size=11))
# SAVE
...
for (k_row in 1:nrow(s_afr_outpatient_cost)) {
    gamma_fit <- c(get.gamma.par(p=ci95_range,
                      q=c(s_afr_outpatient_cost$LCI[k_row],s_afr_outpatient_cost$UCI[k_row]),
                             show.output=F,plot=F),scaling=1)
  # if fitting fails, this can be overcome by dividing the costs by 100 
  # and then scaling the fit 100x afterwards
...
ggplot(s_afr_outpatient_cost) + 
  geom_point(aes(x=mean,y=sim_mean,color=name,fill=cost_type),shape=21,size=3) +
...
if (!any(grepl("shape",colnames(s_afr_inpatient_cost)))){
  ...
  for (k_row in 1:nrow(sa_costs_unique)) {
    ...
    if (any(is.na(fit_gamma))) {
      ...
      fit_gamma <- c(get.gamma.par(q=g_fit_data/100,p=ci95_range,
                                   show.output=F,plot=F)[c("shape","rate")],scaling=100)
      }
...
subfolder_name <- paste0(
        ...
        ifelse(ifelse(exists("flag_adjust_to_mean"),
                      flag_adjust_to_mean,F),"_adj_mv_sev_eff_mean",""),
...
```{r,message=FALSE}
### Before starting loop for the FIRST TIME need to create temp folder 
#   (afterwards can comment out this line)
...
for (k_plot in 1:length(plot_list)) {
  plot_list[[k_plot]] <- plot_list[[k_plot]] + 
    theme(axis.text.x=element_text(vjust=0.5,size=9),
          axis.text.y=element_text(size=8.5),plot.caption=element_text(size=9),
          legend.text=element_text(size=9),legend.background=element_rect(fill=NA),
          legend.position="bottom", # c(0.92,0.925)
...
# calculates ICERs for different price levels 
# (dataframe for default price: df_interv_incremcosts_icer)
...
p <- p_icer_plotlist[[4]] + 
  theme(axis.text.x=element_text(vjust=0.5,size=9),axis.text.y=element_text(size=8.5),
        plot.caption=element_text(size=9),legend.text=element_text(size=9),
...
### Plot for selected prices (Figure 5 in manuscript)
