###### get_burden_flexible ------------------------------------
get_burden_flexible_nonhosp_sari <- function(configList,incid_matrix,hosp_prob_matrix) {
  # set RNG seed
...
# if no maternal efficacy present yet, sample from (log)normal distribution
  ...
  rsv_deaths           <- (hosp_cases + non_hosp_cases)* config$hosp_CFR
  ...
  non_hosp_YLD         <- non_hosp_cases    * config$severe_rsv_DALYloss
  ...
  non_hosp_YLD_averted         <- non_hosp_cases_averted    * config$severe_rsv_DALYloss
