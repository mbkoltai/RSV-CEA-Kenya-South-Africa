...
for (k_daly in c("incremental_cost/DALY_averted","incremental_cost/DALY_disc_averted")) {
  ...
  for (k_plot in 1:2) {
    if (k_plot==2) { df_plot <- df_plot %>% filter(grepl("averted",variable) & grepl("incremental",variable)) }
    df_n_price = df_plot %>% group_by(intervention) %>% summarise(price=unique(price)) %>% group_by(intervention) %>% summarise(n_price=n())
    df_n_price$n_price
...
for (k_scale in 1:length(price_scaling_vect)) {
  ...
  xx <- df_interv_incremcosts_icer[!duplicated(df_interv_incremcosts_icer),] %>% 
    ...
    mutate(value=ifelse(grepl("intervention cost",variable),price_scaling*value,value),
           ...
           cnt_int=paste0(country_iso ," $",price," ",intervention)) %>%
    ...
    arrange(country_iso,intervention)
