p_increm_cost_compar <- ggplot(df_total_DALY_medcost_averted_KEN_ZAF %>% filter(grepl("incremental",variable))) + 
  geom_hpline(aes(x=country_iso,y=median,group=interaction(source,intervention),
            linetype=source),position=position_dodge(width=dodge_val),width=ifelse(exists("width_val"),width_val,0.23),size=1.5) +
  geom_linerange(aes(x=country_iso,ymin=CI95_low,ymax=CI95_high,group=interaction(source,intervention),
       color=price_interv),alpha=0.4,position=position_dodge(width=dodge_val),size=ifelse(exists("linerange_val"),linerange_val,42)) +
  facet_wrap(~variable,scales="free_y",ncol=1) + # ,nrow=2
  scale_color_manual(values=c(colorRampPalette(colors=c("rosybrown","red"))(1),
                              colorRampPalette(colors=c("blue","blueviolet"))(1))) +
  ...
  geom_text(aes(x=country_iso,y=ifelse(median>=0,CI95_high+2,CI95_low-2),
            ...
            position=position_dodge(width=dodge_val),size=ifelse(exists("geom_text_font_size"),geom_text_font_size,7)) +
