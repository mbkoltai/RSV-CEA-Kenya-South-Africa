...
if(0==1)
{
  # RSV_main
  i_scen <- 1
  configList <- as.list(sim_config_matrix[i_scen,])
  # RSV_check_results
  configList <- as.list(scenario_opt[1,col_input])  
}
  
# function to calcuate the maximum price, with the given efficacy, modelType and other parameters
get_burden <- function(configList) {
  ...
  # this loads costs/efficacy but also , b
	###############################
	# Pre-processing              #
	###############################
  ...
  # config$rsv_rate is the dataframs of rsv cases/person adjust to the given country
  ...
  sample_rsv_rate_ind      <- sample(ncol(config$rsv_rate),config$num_sim,replace = T)  
	...
	# this takes 100 samples from the iterations
...
# convert duration of protecion from years to months
  dur_prot_maternal <- round(config$monthsInYear * config$dur_protection_maternal)
	...
	mAgeEffectiveProtection_hospital     <- mAgeEffectiveProtection_primary                          #dummy
	mAgeEffectiveProtection_cfr          <- mAgeEffectiveProtection_primary                          #dummy
	
  # is no maternal efficacy present yet, sample from (log)normal distribution
